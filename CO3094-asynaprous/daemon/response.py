...
class Response():   
    ...
    def prepare_content_type(self, mime_type='text/html'):
        ...
        if main_type == 'text':
            ...
            if sub_type in ['plain', 'css', 'javascript']: # Added javascript support
                base_dir = BASE_DIR + "static/"
            elif sub_type == 'html':
                base_dir = BASE_DIR + "www/"
            elif sub_type in ['csv', 'xml']: # Completed TODO
                base_dir = BASE_DIR + "static/" 
            else:
                base_dir = BASE_DIR + "static/" # Fallback instead of crashing on handle_text_other
            ...
            # --- THE FIX: Route frontend files to static, backend files to apps ---
            if sub_type in ['javascript', 'xml', 'zip', 'json']: # Completed TODO
                base_dir = BASE_DIR + "static/"
            else:
                base_dir = BASE_DIR + "apps/"
                
        elif main_type == 'video': # Completed TODO
            base_dir = BASE_DIR + "static/"
    ...
    def build_response_header(self, request):
        ...
        headers = {
                ...
                "Content-Length": "{}".format(len(self._content) if isinstance(self._content, (bytes, str)) else 0),
        ...
        # --- THE FIX: DYNAMIC STATUS CODES ---
        # Allow the Request or Response to specify the status code, default to 200
        if not getattr(self, 'status_code', None):
            self.status_code = getattr(request, 'status_code', 200)

        # Map common status codes to their reasons
        reasons = {
            200: "OK",
            400: "Bad Request",
            401: "Unauthorized",
        ...
        self.reason = reasons.get(self.status_code or 200, "OK")
        # --------------------------------------
    ...
    def build_response(self, request, envelop_content=None):
        ...
        # --- THE FIX: Hardcode extension checks to bypass Windows Registry bugs ---
        elif path.endswith('.css') or mime_type == 'text/css':
            base_dir = self.prepare_content_type(mime_type = 'text/css')
            
        elif path.endswith('.js') or mime_type in ['application/javascript', 'text/javascript']:
            # Force the mime_type to be correct before passing it to prepare_content_type
